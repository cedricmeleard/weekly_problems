...
describe("truncate with regex test should", () => {
  ...
  test("return 'nev' when passing 'never' and 3", () => {
    expect(_truncate("never", 3)).toBe("nev");
  });

  test("return 'nev nev' when passing 'never nevermind' and 3", () => {
    expect(_truncate("never nevermind", 3)).toBe("nev nev");
  });

  test("return 'never nevermi' when passing 'never nevermind' and 7", () => {
    expect(_truncate("never nevermind", 7)).toBe("never nevermi");
  });

  test("return '*hel* dar, my ~old_fri' when passing '*hello* darkness, my ~old_friend' and 3", () => {
    expect(_truncate("*hello* darkness, my ~old_friend", 3)).toBe("*hel* dar, my ~old_fri");
  });
